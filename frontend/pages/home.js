                ...
                <div class="expense-category-details hidden" id="category-details-${safeCategoryId}">
                  ${sortedMovements.map(movement => `
                    <div class="movement-detail-entry" data-movement-id="${movement.id}">
                      ...
                      <div class="entry-actions">
                        ${(movement.generated_from_template_id && !movement.payer_user_id && !movement.payer_contact_id)
                          ? `<span class="entry-autogenerated-badge" title="Generado automáticamente">⚡ Auto</span>`
                          : ''
                        }
                        ${movement.is_split
...
async function handleEditMovement(movementId) {
  // Check if this movement is pocket-linked
  const dataSource = originalMovementsData || movementsData;
  const movement = dataSource?.movements?.find(m => m.id === movementId);
...
async function handleDeleteMovement(movementId) {
  ...
  if (movement?.source_pocket_id) {
    confirmed = await showConfirmation(
      ...
      `Este gasto está vinculado al bolsillo "${movement.source_pocket_name}". Al eliminarlo también se eliminará la transacción del bolsillo. ¿Continuar?`
    );
  } else {
    confirmed = await showConfirmation(
      '¿Eliminar gasto?',
...
/**
 * Find a movement by ID in the loaded data
 */
...
/**
 * Handle Escape key to close movement detail modal
 */
function handleMovementDetailEscape(e) {
  if (e.key === 'Escape') closeMovementDetailModal();
}

/**
 * Close the movement detail modal with animation
 */
function closeMovementDetailModal() {
  ...
  document.removeEventListener('keydown', handleMovementDetailEscape);
}

/**
 * Show read-only movement detail modal
 */
function showMovementDetailModal(movement) {
  // Close any existing modal first
  const existing = document.getElementById('movement-detail-modal');
  if (existing) existing.remove();

  // Determine badges
  const isAutoGenerated = movement.generated_from_template_id && !movement.payer_user_id && !movement.payer_contact_id;
  const isSplit = movement.is_split;
  const hasPocket = !!movement.source_pocket_name;
  // Detect loan: SPLIT with exactly 1 participant at 100%
  const isLoan = isSplit && movement.participants && movement.participants.length === 1
    && Math.round(movement.participants[0].percentage * 100) === 100;

  const badgesHtml = (isSplit || isAutoGenerated || hasPocket) ? `
    <div class="detail-badges">
      ${isSplit ? (isLoan
        ? '<span class="entry-loan-badge">Préstamo</span>'
        : '<span class="entry-split-badge">Compartido</span>') : ''}
      ${isAutoGenerated ? '<span class="entry-autogenerated-badge">⚡ Auto</span>' : ''}
      ${hasPocket ? `<span class="entry-payment-badge">📥 ${movement.source_pocket_name}</span>` : ''}
    </div>
  ` : '';
  ...
  if (movement.description) {
    fieldsHtml += `
      <div class="detail-field">
        ...
        <span class="detail-value">${movement.description}</span>
      </div>`;
  }
  ...
  if (movement.movement_date) {
    fieldsHtml += `
      <div class="detail-field">
        ...
        <span class="detail-value">${formatDate(movement.movement_date)}</span>
      </div>`;
  }

  // Category with group name (no icon)
  const categoryDisplay = movement.category_name || movement.category;
  if (categoryDisplay) {
    const groupName = movement.category_group_name || '';
    fieldsHtml += `
      <div class="detail-field">
        <span class="detail-label">Categoría</span>
        <span class="detail-value">${groupName}${groupName ? ' › ' : ''}${categoryDisplay}</span>
      </div>`;
  }

  // Payer — skip for HOUSEHOLD (payment method is enough), show for SPLIT/LOAN
  if (movement.payer_name && isSplit) {
    const payerLabel = isLoan ? 'Quién prestó' : 'Quién pagó';
    fieldsHtml += `
      <div class="detail-field">
        <span class="detail-label">${payerLabel}</span>
        <span class="detail-value">${movement.payer_name}</span>
      </div>`;
  }

  // For loans: show who received the loan (the single participant)
  if (isLoan && movement.participants[0]) {
    fieldsHtml += `
      <div class="detail-field">
        <span class="detail-label">Recibió</span>
        <span class="detail-value">${movement.participants[0].participant_name}</span>
      </div>`;
  }

  // Payment method
  if (movement.payment_method_name) {
    fieldsHtml += `
      <div class="detail-field">
        <span class="detail-label">Método de pago</span>
        <span class="detail-value">${movement.payment_method_name}</span>
      </div>`;
  }

  // Participants section (only for SPLIT with multiple participants, not loans)
  let participantsHtml = '';
  if (isSplit && !isLoan && movement.participants && movement.participants.length > 0) {
    const participantRows = movement.participants.map(p => {
      const pct = Math.round(p.percentage * 100);
      const amount = p.amount != null
        ? formatCurrency(p.amount)
        : formatCurrency(movement.amount * p.percentage);
      return `
        <div class="detail-participant-row">
          <div class="participant-info">
            <span class="participant-name">${p.participant_name}</span>
            <span class="participant-pct">${pct}%</span>
          </div>
    ...
    <div class="modal-overlay modal-animate-in" id="movement-detail-modal">
      <div class="modal modal-slide-up">
        <div class="modal-header">
          ...
          <button class="modal-close-btn" id="detail-close-btn">&times;</button>
        </div>
        <div class="modal-body">
          ${badgesHtml}
          <div class="detail-amount">${formatCurrency(movement.amount)}</div>
        ...
        <div class="modal-footer">
          <button class="btn-secondary" id="detail-edit-btn">Editar</button>
          <button class="btn-danger" id="detail-delete-btn">Eliminar</button>
        </div>
      </div>
    </div>
  `;
  ...
  const overlay = document.getElementById('movement-detail-modal');

  // Close button
  document.getElementById('detail-close-btn').addEventListener('click', closeMovementDetailModal);
  ...
  // Escape key to close
  document.addEventListener('keydown', handleMovementDetailEscape);
...
function setupCategoryListeners() {
  ...
  // Click on movement row to open detail modal (gastos tab)
  document.querySelectorAll('.movement-detail-entry[data-movement-id], .chronological-movement-card[data-movement-id]').forEach(row => {
    row.addEventListener('click', (e) => {
      // Don't open modal if click was on three-dots button or menu
